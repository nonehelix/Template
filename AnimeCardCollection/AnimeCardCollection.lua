return {
	Load = function(Shared)
		...
		do
			...
			function Feature:ResetCycleProgress()
				table.clear(self.State.RolledThisCycle)
			end
			...
			function Feature:CanRollCardNow(cardId)
				local lastRollAt = self.State.LastRollAtByCard[cardId]
				if not lastRollAt then
					return true
				end

				return (tick() - lastRollAt) >= self.State.PerCardCooldown
			end

			function Feature:HasAvailableCardThisCycle()
				local ownedCards = self:GetOwnedCards()

				for _, cardId in ipairs(self.State.Queue) do
					if
						not self:IsCardDone(cardId)
						and ownedCards[cardId] ~= nil
						and not self.State.RolledThisCycle[cardId]
						and self:CanRollCardNow(cardId)
					then
						return true
					end
				end
			...
			function Feature:GetNextCardToRoll()
				...
				if not self:HasAvailableCardThisCycle() then
					self:ResetCycleProgress()
				end
				...
				for _ = 1, count do
					...
					if
						cardId
						and not self:IsCardDone(cardId)
						and ownedCards[cardId] ~= nil
						and not self.State.RolledThisCycle[cardId]
						and self:CanRollCardNow(cardId)
					then
			...
			function Feature:SendGradeRoll(cardId)
				self.State.LastRollAtByCard[cardId] = tick()
				self.State.RolledThisCycle[cardId] = true
			...
			function Feature:Start(panelRef)
				...
				table.clear(self.State.LastRollAtByCard)
				self:ResetCycleProgress()
