return {
	Load = function(Shared)
		...
		local function getEnemyDisplayName(enemy)
			...
			local main = healthBar and healthBar:FindFirstChild("Main") or nil
		...
		local enemyDisplayMappingsCache = nil

		local function getEnemyDisplayMappings()
			if enemyDisplayMappingsCache ~= nil then
			...
			if type(enemyInfo) == "table" then
				local function addLookup(candidate, displayName)
					...
					if key == "" or displayName == "" then
						return
					end

					local allNames = allLookup[key]
					if not allNames then
						allNames = {}
						allLookup[key] = allNames
					end

					if not arrayContains(allNames, displayName) then
						allNames[#allNames + 1] = displayName
					end
					...
					if uniqueLookup[key] == nil then
						uniqueLookup[key] = displayName
					elseif uniqueLookup[key] ~= displayName then
						uniqueLookup[key] = nil
						duplicates[key] = true
					end
				end
		...
		local function getServerEnemy(enemy)
			...
			if enemy == serverFolder then return nil end
		...
		local function getServerEnemyCandidates()
			...
			local function addCandidate(enemy)
				...
				local serverEnemy = getServerEnemy(enemy)
				if not serverEnemy or seen[serverEnemy] then return end
				...
				candidates[#candidates + 1] = serverEnemy
			end
		...
		local function buildAutoFarmZoneData()
			local mapInfo = requireIndexerModule({"MapInfo", "MapsInfo"})
			...
			local zoneEnemies = {}
			local internalNameMap = getEnemyDisplayMappings().All
			local zoneEntries = {}
			...
			for _, entry in ipairs(zoneEntries) do
				...
				collectInternalNames(getZoneNormalList(entry.Info), internalNames, {})
		...
		local function loadAutoFarmZoneData()
			if autoFarmZoneDataLoaded then
				return
			end

			local ok, zoneItems, zoneEnemies = pcall(buildAutoFarmZoneData)
			if ok and type(zoneItems) == "table" and type(zoneEnemies) == "table" then
				autoFarmZoneItemsCache = zoneItems
				autoFarmZoneEnemiesCache = zoneEnemies
				autoFarmZoneDataLoaded = true
			elseif not ok then
				warn("[AriseCrossover] Failed to load Auto Farm zone data: " .. tostring(zoneItems))
			end
		end
		...
		local function teleportToSavedLocation(selectedLocation)
			local cframe = TELEPORT_STATIC_LOCATIONS[selectedLocation]
			if not cframe then
				local spawnInstance = teleportSpawnNameToInstance[selectedLocation]
				if not (spawnInstance and spawnInstance.Parent) then
					local spawnsFolder = getTeleportSpawnsFolder()
					spawnInstance = spawnsFolder and spawnsFolder:FindFirstChild(selectedLocation) or nil
				end
				cframe = getTeleportSpawnCFrame(spawnInstance)
			end
		...
		local function getLiveDungeonInfo(dungeonObject)
			...
			local ok, attrs = pcall(function()
				return dungeonObject:GetAttributes()
			end)
			...
			if not hasCoreInfo or not hasIdentifier then return nil end
		...
		local function isDungeonEndingNow()
			...
			local dungeonInfoLabel = dungeonInfo and dungeonInfo:FindFirstChild("TextLabel") or nil
		...
		do
			...
			function Feature:MatchesSelection(enemy, selectedEnemies)
				...
				for _, candidate in ipairs({
					...
					if arrayContains(selectedEnemies, candidate) then
						return true
					end
				end
		...
		do
			...
			function Feature:Run(panelRef)
				...
				runFeatureTask(self, "Starting", function()
					...
					ownTimeTrial = getOwnTimeTrialInfo() or waitForInfo(getOwnTimeTrialInfo, DUNGEON_INSTANCE_WAIT)
		...
		do
			...
			Feature.Cleanup = buildTaskCleanup("Teleporting")
		end
