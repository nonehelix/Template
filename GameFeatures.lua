...
return function(Shared)
    -- Get required functions from Shared
    local RegisterFeature = Shared.RegisterFeature
    local arrayContains   = Shared.arrayContains or function(t, v) 
        if not t then return false end
        for _, val in ipairs(t) do if val == v then return true end end
        return false 
    end

    local Workspace = Shared.Workspace or workspace
    ...
    -- Return the final compiled config from Shared
